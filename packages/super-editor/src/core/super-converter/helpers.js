...
const applyThemeTint = (hexColor, tintHex) => {
  if (!tintHex) return hexColor;
  const tintValue = Number.parseInt(tintHex, 16);
  if (!Number.isFinite(tintValue)) return hexColor;
  const rgb = hexToRgb(hexColor);
  if (!rgb) return hexColor;
  const factor = 1 - tintValue / 255;
  const r = Math.round(rgb.r + (255 - rgb.r) * factor);
  const g = Math.round(rgb.g + (255 - rgb.g) * factor);
  const b = Math.round(rgb.b + (255 - rgb.b) * factor);
  const toByte = (n) => Math.max(0, Math.min(255, n)).toString(16).padStart(2, '0').toUpperCase();
  return `${toByte(r)}${toByte(g)}${toByte(b)}`;
};

const applyThemeShade = (hexColor, shadeHex) => {
  if (!shadeHex) return hexColor;
  const shadeValue = Number.parseInt(shadeHex, 16);
  if (!Number.isFinite(shadeValue)) return hexColor;
  const rgb = hexToRgb(hexColor);
  if (!rgb) return hexColor;
  const factor = shadeValue / 255;
...
const resolveShadingFillColor = (shading, themeColors = null) => {
  if (!shading || typeof shading !== 'object') return null;

  let fill = normalizeHexColor(shading.fill);

  if (!fill && shading.themeFill && themeColors) {
    const themeColor = themeColors[shading.themeFill];
    if (themeColor) {
      fill = normalizeHexColor(themeColor);
      if (fill && shading.themeFillTint) {
        fill = applyThemeTint(fill, shading.themeFillTint);
      }
      if (fill && shading.themeFillShade) {
        fill = applyThemeShade(fill, shading.themeFillShade);
      }
    }
  }
